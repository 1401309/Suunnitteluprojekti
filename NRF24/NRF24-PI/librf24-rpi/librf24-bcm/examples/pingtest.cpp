...
int main(int argc, char** argv)
{
	...
	while (true)
	{
		...
		{	
			...
			{
				printf("Radio with id %d not identified", radio_id);
			}
		}
	}
...
void SaveValues()
{
	...
	signed char Temp;
	unsigned char Hum;
	FILE * fd = fopen("testi.txt", "rw+");
	
	for(int index=0;index<SENSOR_COUNT; index++)
	{
	Temp = Temperatures[index];
	Hum = Humidities[index];
	memcpy(&SensorInfo[index*4], &radio_ids[index], 2);
	memcpy(&SensorInfo[index*4 + 2], &Temp, 1);
	memcpy(&SensorInfo[index*4 + 3], &Hum, 1);
	memcpy(&SensorInfo[index*4 + 4], &Times[index], 20);
	}
